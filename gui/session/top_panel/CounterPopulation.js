...
class CounterPopulation
{
	constructor(resCode, panel, icon, count, stats)
	{
		...
		this.playerCiv = Engine.GuiInterfaceCall("GetPlayerCiv", playerID); 
	}
	...
	rebuild(playerState, getAllyStatTooltip)
	{
		...
		this.panel.tooltip =
			setStringTags(translate(this.playerCiv == "robot"? this.RobotPopulationTooltip : this.HumanPopulationTooltip), CounterManager.ResourceTitleTags) +
...
CounterPopulation.prototype.RobotPopulationTooltip = markForTranslation("Energy: current/limit (max)");

CounterPopulation.prototype.HumanPopulationTooltip = markForTranslation("Population: current/limit (max)");
