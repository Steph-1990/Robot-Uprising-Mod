...
function displaySingle(entState)
{
	...
	if (showHealth)
	{
		...
		Engine.GetGUIObjectByName("health").tooltip = sprintf(translate("Health"));
	...
	if (entState.capturePoints)
	{
		...
		Engine.GetGUIObjectByName("capture").tooltip = sprintf(translate("Capture"));
	}
	...
	if (entState.promotion)
	{
		...
		experienceSize.rright =(100 * Math.max(0, Math.min(1, 1.0 * +entState.promotion.curr / (+entState.promotion.req || 1))));
	...
	if (entState.resourceSupply)
	{
		...
		Engine.GetGUIObjectByName("resources").tooltip = sprintf(translate("Resources"));
...
function updateSelectionDetails()
{
	...
	let marketPanel = Engine.GetGUIObjectByName("marketCommands");
