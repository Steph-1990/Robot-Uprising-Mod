 ...
 */
var g_FlareSound = "audio/interface/alarm/alarm_starcilite_crash.ogg";
...
var g_UnitActions =
{
	...
	"drop-units":
	{
		...
		{				
			...
			DrawTargetMarker(target); // Petit croix qui s'affiche lorsqu'on clique à un endroit

			/*Engine.GuiInterfaceCall("PlaySound", {  // Son de l'unité lorsqu'on lui demande d'aller patrouiller
				"name": "order_patrol",
				"entity": action.firstAbleEntity
			});*/
		...
		"hotkeyActionCheck": function(target, selection)
		{
			return Engine.HotkeyIsPressed("session.dropunits") &&
				this.actionCheck(target, selection);
		},
		"preSelectedActionCheck": function(target, selection)
		{
			return preSelectedAction == ACTION_DROP_UNITS &&
				this.actionCheck(target, selection);
		},
		"actionCheck": function(target, selection)
		{
			let actionInfo = getActionInfo("drop-units", target, selection);
			return actionInfo.possible && {
				"type": "drop-units",
				"cursor": "action-dropUnits",
				"firstAbleEntity": actionInfo.entity
			};
		},
		"specificness": 60,
	...
	"gather":
	{
		...
		"getActionInfo": function(entState, targetState)
		{
			...
			if (!resource || ((!playerCheck(entState, targetState, ["Player", "Ally"]) && targetState.resourceSupply.type.generic != "metal")))
				return false;
...
var g_EntityCommands =
{
	...
	"drop-units": {
		"getInfo": function(entStates)
		{
			const classes = ["SpaceshipUnitsTransporter"];
			if (entStates.every(entState => !MatchesClassList(entState.identity.classes, classes)))
				return false;

			return {
				"tooltip": colorizeHotkey("%(hotkey)s" + " ", "session.dropunits") +
				           translate("Drop units at specified location."),
				"icon": "dropUnits.png",
				"enabled": true
			};
		},
		"execute": function(entStates)
		{
			...
			preSelectedAction = ACTION_DROP_UNITS;
		},
		"allowedPlayers": ["Player"]
	},
