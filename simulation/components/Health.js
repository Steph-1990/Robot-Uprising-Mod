...
Health.prototype.EjectOnFullHP = function()
{
	let cmpGarrisonable = Engine.QueryInterface(this.entity, IID_Garrisonable);

	if (cmpGarrisonable)
	{
		let HolderID = cmpGarrisonable.HolderID();
		let cmpIdentity = Engine.QueryInterface(HolderID, IID_Identity);
		let cmpGarrisonHolder = Engine.QueryInterface(HolderID, IID_GarrisonHolder);

		if (cmpGarrisonable.IsGarrisoned() && this.hitpoints == this.maxHitpoints)
		{
			if (cmpIdentity.HasClass("TechnicalCenter"))
				cmpGarrisonable.UnGarrison(true);	
		}
	}	
};
