...
/**
 * Extra Schema for Robots to be able to apply to anything.
 */
BuildRestrictions.prototype.AppliesToSchema =  "<optional>" +
    "<element name='AppliesTo' a:help='Specifies entity owner restrictions for this building.'>" +
        "<choice>" +
            "<value>all</value>" +
            "<value>own</value>" +
            "<value>ally</value>" +
...
BuildRestrictions.prototype.DrillAroundSchema =  "<optional>" +
    "<element name='DrillAround' a:help='Specifies if there is a drill around.'>" +
		"<data type='boolean'/>" +
    "</element>" +
	"<optional><element name='MinDrillDistance'><data type='positiveInteger'/></element></optional>" +
"</optional>";
...
BuildRestrictions.prototype.Schema =
	...
	"<optional>" +
		"<element name='Distance' a:help='Specifies distance restrictions on this building, relative to buildings from the given category.'>" +
			"<interleave>" +
				BuildRestrictions.prototype.AppliesToSchema +
				BuildRestrictions.prototype.DrillAroundSchema +
...
BuildRestrictions.prototype.CheckPlacement = function()
{
	...
	let usableEnergy = (cmpPlayer.GetPopulationLimit() - cmpPlayer.GetPopulationCount()); 

	if (usableEnergy < 0 && (cmpIdentity.HasClass("Generator") == false)) 
	{
		result.message = markForTranslation("%(name)s cannot be built due to lack of energy");
		return result;
	}
	...
	if (this.template.Distance)
	{
		...
		const filter = (id) => Engine.QueryInterface(id, IID_Identity)?.HasClass(cat);
		...
		if (this.template.Distance.MinDistance !== undefined)
		{
			...
			if (cmpRangeManager.ExecuteQuery(this.entity, 0, minDistance, this.GetAffectedPlayers(cmpPlayer), IID_BuildRestrictions, false).some(filter))
			{
		...
		if (this.template.Distance.MaxDistance !== undefined)
		{
			...
			if (!cmpRangeManager.ExecuteQuery(this.entity, 0, maxDistance, this.GetAffectedPlayers(cmpPlayer), IID_BuildRestrictions, false).some(filter))
			{
				let result = markForPluralTranslation(
		...
		// Ceci est un objet drill, on ne peut pas le placer si une drill est proche.
		if (this.template.Distance.DrillAround === "true")
		{
			const drillFilter = (id) => Engine.QueryInterface(id, IID_Identity)?.HasClass("Drill");
			// Si on a une drill min distance on prend ça sinon on prend la min distance.
			const drillMinDistance = +this.template.Distance.MinDrillDistance;
			const templateMinDistance = drillMinDistance > 0 ? drillMinDistance : +this.template.Distance.MinDistance;
			let minDistance = ApplyValueModificationsToTemplate("BuildRestrictions/Distance/MinDrillDistance", templateMinDistance, cmpPlayer.GetPlayerID(), template);
			if (cmpRangeManager.ExecuteQuery(this.entity, 0, minDistance, this.GetAffectedPlayers(cmpPlayer), IID_BuildRestrictions, false).some(drillFilter))
			{
				let result = markForPluralTranslation(
					"%(name)s too close to a drill, must be at least %(distance)s meter away",
					"%(name)s too close to a drill, must be at least %(distance)s meters away",
					minDistance);
...
BuildRestrictions.prototype.GetMaxDistance = function()
{
	return this.template.Distance.MaxDistance;
};
