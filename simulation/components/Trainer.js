...
Trainer.prototype.Item.prototype.Spawn = function()
{
	...
	const cmpIdentity = Engine.QueryInterface(this.trainer, IID_Identity);
	const cmpGarrisonHolder = Engine.QueryInterface(this.trainer, IID_GarrisonHolder);	

	if ((cmpIdentity.HasClass("MobileInfantryFactory") || cmpIdentity.HasClass("MobileArmoredInfantryFactory")) && !cmpGarrisonHolder.IsFull())
		autoGarrison = true;
	...
	while (this.entities.length)
	{
		...
		if (!garrisoned)
		{
			...
			if (pos.y < 0 || cmpIdentity.HasClass("MobileInfantryFactory") || cmpIdentity.HasClass("MobileArmoredInfantryFactory"))
				break;
