...
ResourceSupply.prototype.Init = function()
{
	...
	if (Engine.QueryInterface(this.entity, IID_Identity).HasClass("CopperDrill"))
		resourcesType = "Copper";
	else if (Engine.QueryInterface(this.entity, IID_Identity).HasClass("TitaniumDrill"))
		resourcesType = "Titanium";
	else if (Engine.QueryInterface(this.entity, IID_Identity).HasClass("QuartzDrill"))
		resourcesType = "Quartz";

	if (resourcesType != "")
	{
		let cmpRangeManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager);

		this.maxDistance = Engine.QueryInterface(this.entity, IID_BuildRestrictions).GetMaxDistance();
		this.Resources = cmpRangeManager.GetGaiaAndNonGaiaEntities().filter(e => {
		let cmpIdentity = Engine.QueryInterface(e, IID_Identity);
		return cmpIdentity && cmpIdentity.HasClass(resourcesType);
		});
	}
};
...
ResourceSupply.prototype.Change = function(change)
{
	...
	{
		if(PositionHelper.DistanceBetweenEntities(this.entity, this.Resources[i]) <= this.maxDistance)
				Engine.QueryInterface(this.Resources[i], IID_Health).SetHitpoints(this.GetCurrentAmount()-1);

		if ((this.GetCurrentAmount()-1) == 0)
			Engine.QueryInterface(this.Resources[i], IID_Health).Kill();
	}
	...
	if (this.amount == 0 && cmpIdentity.HasClass("Starcilite"))
...
ResourceSupply.prototype.OnOwnershipChanged = function(msg)
{
	...
	{
		if(PositionHelper.DistanceBetweenEntities(this.entity, this.Resources[i]) < this.maxDistance)
				this.amount = Engine.QueryInterface(this.Resources[i], IID_Health).GetHitpoints();
	}
