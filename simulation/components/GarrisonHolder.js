...
GarrisonHolder.prototype.Garrison = function(entity)
{
	...
	if (cmpHealth.GetHitpoints() == cmpHealth.GetMaxHitpoints() && cmpIdentity.HasClass("TechnicalCenter")) 
		return false;
...
GarrisonHolder.prototype.Unload = function(entity)
{
	...
	if	(cmpIdentity.HasClass("Spaceship") && !cmpUnitMotionFlying.IsOnGround())
	{
		cmpUnitMotionFlying.SetLanding(true);
		cmpUnitAI.DropAfterLanding();
		return true;
	}

	let cmpGarrisonable = Engine.QueryInterface(entity, IID_Garrisonable);
	if (cmpIdentity.HasClass("TechnicalCenter") || cmpIdentity.HasClass("MobileTechnicalCenter"))
		return cmpGarrisonable && cmpGarrisonable.UnGarrison(true);
	else
