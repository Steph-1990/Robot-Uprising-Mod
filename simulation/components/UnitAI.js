...
UnitAI.prototype.UnitFsmSpec = {
	...
	"Order.Stop": function (msg) {
		...
		if	(cmpIdentity.HasClass("Spaceship"))
		{
			this.dropUnits = false;
	...
	"Order.DropUnits": function (msg) {
		...
		this.SetNextState("INDIVIDUAL.WALKINGANDDROPING");
		return ACCEPT_ORDER;
	},
	...
	"Order.PickupUnit": function(msg) {
		...
		if (cmpPassengerMotion && cmpPassengerMotion.IsTargetRangeReachable(this.entity, range.min, range.max) 
		&& PositionHelper.DistanceBetweenEntities(this.entity, msg.data.target) < 300)
	...
	"Order.Garrison": function(msg) {
		...
		let cmpUnitMotionFlying = Engine.QueryInterface(msg.data.target, IID_UnitMotion);

		if (this.CheckTargetRange(msg.data.target, msg.data.garrison ? IID_Garrisonable : IID_Turretable) && cmpUnitMotionFlying && cmpUnitMotionFlying.IsOnGround()) 
	...
	"INDIVIDUAL": {
		...
		"IDLE": {
			...
			"enter": function() {
				...
				if (cmpIdentity.HasClass("Hunter"))
					this.SelectAnimation("idle_spaceship", true);
				else if (!cmpIdentity.HasClass("MobileStructure"))
			...
			"Unload": function(msg) {
				...
				if (cmpGarrisonHolder && this.dropUnits)
				{
			...
			"Upgrade": function(msg) {
				...
				{	
					...
					let upgradeTemplates = cmpUpgrade.GetUpgrades();
					cmpUpgrade.Upgrade(upgradeTemplates[0].entity);
					this.upgrading = false;	
				}	
			},
		...
		"WALKING": {
			"enter": function() {
				...
				if (cmpIdentity.HasClass("Hunter"))
					this.SelectAnimation("walk", true);
			...
			"MovementUpdate": function(msg) {
				...
				|| this.CheckRange(this.order.data) || cmpIdentity.HasClass("Spaceship"))
				{
					if (this.tutorialRequest)
					{
						let cmpTrigger = Engine.QueryInterface(SYSTEM_ENTITY, IID_Trigger);
						cmpTrigger.CallEvent("OnEntityReachedTarget", {"entity": this.entity});
					}
					this.FinishOrder();
				}
			},
		...
		"WALKINGANDDROPING": {
				...
				let cmpIdentity = Engine.QueryInterface(this.entity, IID_Identity);
				if (msg.likelyFailure || msg.obstructed && this.RelaxedMaxRangeCheck(this.order.data, this.DefaultRelaxedMaxRange) 
				|| this.CheckRange(this.order.data) || cmpIdentity.HasClass("Spaceship") == true)
					this.FinishOrder();
				},
			},
		...
		"COMBAT": {
			...
			"ATTACKING": {
				"enter": function() {
					...
					let cmpUnitMotionFlying = Engine.QueryInterface(this.entity, IID_UnitMotion);

					if (cmpIdentity.HasClass("Spaceship"))
					    cmpUnitMotionFlying.SetHoveringFlight();	

						
					if (cmpIdentity.HasClass("Kamikaze"))
						cmpHealth.Kill();
		...
		"GARRISON": {
			"APPROACHING": {
				...
				"MovementUpdate": function(msg) {
					...
					if (this.CheckTargetRange(this.order.data.target, this.order.data.garrison ? IID_Garrisonable : IID_Turretable) && !cmpIdentity.HasClass("Spaceship"))
						this.SetNextState("GARRISONING");
					...
					else
					{
						if (this.pickup)
						{
							let cmpUnitMotionFlying = Engine.QueryInterface(this.pickup, IID_UnitMotion);	
							let cmpUnitAI = Engine.QueryInterface(this.pickup, IID_UnitAI);

							if (cmpUnitMotionFlying.IsOnGround() && this.CheckTargetRange(this.order.data.target, this.order.data.garrison ? IID_Garrisonable : IID_Turretable))
							{
								let cmpGarrisonable = Engine.QueryInterface(this.entity, IID_Garrisonable);
								cmpGarrisonable.Garrison(this.order.data.target);
								if(cmpGarrisonable.IsGarrisoned())
									Engine.PostMessage(this.pickup, MT_PickupCanceled, { "entity": this.entity });
							}
...
UnitAI.prototype.OnUpdate = function ()
{
	...
	// if (cmpIdentity.HasClass("SpaceshipUnitsTransporter") && this.order)
	// {
	// 		warn (this.order.type)
	// }

	// if (cmpIdentity.HasClass("Hunter"))
	// {
	// 	warn(this.GetCurrentState().split(".").pop());
	// } 	
 }
...
UnitAI.prototype.StopMoving = function()
{
	...
	if (cmpUnitMotion && !cmpIdentity.HasClass("Spaceship"))
		cmpUnitMotion.StopMoving();
};

UnitAI.prototype.Landing = function()
{
	let cmpUnitMotionFlying = Engine.QueryInterface(this.entity, IID_UnitMotion);

	if	(cmpUnitMotionFlying)
		cmpUnitMotionFlying.SetLanding(true);
}

UnitAI.prototype.TakeOff = function()
{
	let cmpUnitMotionFlying = Engine.QueryInterface(this.entity, IID_UnitMotion);

	if	(cmpUnitMotionFlying)
		cmpUnitMotionFlying.SetLanding(false);
}
...
UnitAI.prototype.IsOnGroundNotification = function()
{
	this.UnitFsm.ProcessMessage(this, { "type": "Unload" });

	if (this.upgrading)
		this.UnitFsm.ProcessMessage(this, { "type": "Upgrade" });
};

UnitAI.prototype.DropAfterLanding = function()
{
	this.dropUnits = true;
...
UnitAI.prototype.UpgradeAfterLanding = function()
{	
	if (this.order)
		this.FinishOrder();
...
UnitAI.prototype.FaceTowardsTarget = function(target)
{
	...
	if (cmpUnitMotion && !cmpIdentity.HasClass("Hunter"))
	{
	...
	if (cmpIdentity.HasClass("Hunter"))
		{
			...
			cmpPosition.SetYRotation(Math.atan2(targetPosition.x - cmpPosition.GetPosition2D().x, targetPosition.y - cmpPosition.GetPosition2D().y));
		}
...
UnitAI.prototype.CanDropUnits = function()
{
	// Formation controllers should always respond to commands
	// (then the individual units can make up their own minds)
	if (this.IsFormationController())
	...
	return this.template.CanDropUnits == "true";
};
...
UnitAI.prototype.DropUnits = function(x, z, queued, pushFront)
{
	this.AddOrder("DropUnits", { "x": x, "z": z,"force": true }, queued, pushFront);
};
...
UnitAI.prototype.RequestFromTutorialScript = function()
{
	this.tutorialRequest = true;
}

UnitAI.prototype.CancelRequest = function()
{
	this.tutorialRequest = false;
}
