...
UnitMotionFlying.prototype.Init = function()
{
	...
	this.groundValueSaved = false;
	this.hasReachedFlightHeight = false;
	this.groundAtStart = 0;
...
UnitMotionFlying.prototype.OnUpdate = function(msg)
{
	...
	if (!this.hasTarget || cmpGarrisonable.IsGarrisoned()) 
		return;
	...
	if (!this.groundValueSaved)
	{
		this.groundAtStart = ground;
		this.groundValueSaved = true;
	}
	...
	if (this.landing)
	{
		...
		else if (this.speed == 0 && this.onGround)
		{
			...
			{
				cmpUnitAI.IsOnGroundNotification();	
				this.pitch = 0;
 				// We've stopped.	
				canTurn = false;
				this.hasTarget = false;
		...
		{		
			...
			if ((pos.y - targetHeight) / this.template.FlyingHeight > 1)
				this.pitch = 0;
			else
				this.pitch = this.template.TiltRate; 
	...
	else
	{
		...
		else
		{
			...
			this.speed = Math.min(this.template.MaxSpeed, this.speed + turnLength * this.template.AccelRate) * this.speedCoef;
			let standardFlyingHeight = this.groundAtStart + (+this.template.FlyingHeight);


			if (this.hasReachedFlightHeight)
			{
				if (ground+10 >= standardFlyingHeight)
				{
					pos.y = Math.min(ground+5, pos.y + turnLength * this.template.ClimbRate);
					cmpPosition.SetHeightRelative(true);
				}
				else
				{
					pos.y = standardFlyingHeight;
					cmpPosition.SetHeightRelative(false);
				}
			}
			else
			{
				if (ground+10 < standardFlyingHeight)
				{
					pos.y = Math.min(standardFlyingHeight, pos.y + turnLength * this.template.ClimbRate);
					...
					if (pos.y >= standardFlyingHeight)
						this.hasReachedFlightHeight = true;
				}
			}
		}
	}
	...
	let isBehindTarget = ((this.targetX - pos.x) * Math.sin(angle) + (this.targetZ - pos.z) * Math.cos(angle) < 5);
	...
	if (cmpIdentity.HasClass("MobileStructure"))
		cmpPosition.SetXZRotation(0,0);
	else
		cmpPosition.SetXZRotation(this.pitch, this.roll);

	let distanceFromMapCenter = Math.hypot(Math.abs(pos.x-terrainSize/2), Math.abs(pos.z-terrainSize/2));
	let mapRadiusWithMarge = terrainSize/2.1;

	if (distanceFromMapCenter < mapRadiusWithMarge)
	{
...
UnitMotionFlying.prototype.OnValueModification = function(msg)
{
	if (msg.component == "UnitMotionFlying")
		this.RecalculateValues();
};

UnitMotionFlying.prototype.SetHoveringFlight = function()
{
	this.hasTarget = false;
};

UnitMotionFlying.prototype.RecalculateValues = function()
{	
	let newSpeed = ApplyValueModificationsToEntity("UnitMotionFlying/MaxSpeed", +this.template.MaxSpeed, this.entity);

	this.speedCoef = newSpeed / + this.template.MaxSpeed;
};
...
UnitMotionFlying.prototype.IsOnGround = function()
{
...
{
	//warn(new Error().stack); // Utile pour savoir quel ordre de UnitAI appelle la fonction StopMoving()
	if (!this.waterDeath)
		this.landing = !this.landing;

};

UnitMotionFlying.prototype.CheckIsLanding = function()
{
	return this.landing;
};

UnitMotionFlying.prototype.SetLanding = function(landing)
{
