...
BuildingAI.prototype.OnRangeUpdate = function(msg)
{
	...
	{	
		if (cmpAttack.CanAttack(entity) && cmpIdentity.HasClass("Landmine"))
			cmpHealth.Kill();
		else if (cmpAttack.CanAttack(entity))
...
BuildingAI.prototype.FireArrows = function()
{
	...
	{
		if (cmpIdentity && cmpIdentity.HasClass("Tower") && cmpVisual)
	...
	while (firedArrows < arrowsToFire && targets.length())
	{
		...
		{
			if (cmpIdentity && cmpIdentity.HasClass("Tower") && cmpVisual)
				cmpVisual.SelectAnimation("attack_ranged", false, 1.0);
