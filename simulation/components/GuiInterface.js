 ...
 */
GuiInterface.prototype.SetEntitiesSelection = function(player, data)
{
	let cmpTrigger = Engine.QueryInterface(SYSTEM_ENTITY, IID_Trigger);
	cmpTrigger.CallEvent("OnEntitiesSelected", { "player": player, "data": data});
};

GuiInterface.prototype.SetTutorialIsLaunched = function(tutorialIsLaunched)
{
	this.tutorialIsLaunched = tutorialIsLaunched;
};

GuiInterface.prototype.GetTutorialIsLaunched = function()
{
	return this.tutorialIsLaunched;
};
...
GuiInterface.prototype.GetEntityState = function(player, ent)
{
	...
	if (cmpUnitAI)
		ret.unitAI = {
			...
			"canDropUnits": cmpUnitAI.CanDropUnits(),
...
GuiInterface.prototype.GetPlayerCiv = function(player)
{
	let playerCiv = QueryPlayerIDInterface(player, IID_Identity).GetCiv();
	return playerCiv;
};
...
GuiInterface.prototype.DisplayArrow = function(entity, state)
{
	if (this.tutorialIsLaunched)
	{
		...
		if (cmpIdentity.HasClass("CopperDrill"))
			classe = "Copper";
		else if (cmpIdentity.HasClass("TitaniumDrill"))
			classe = "Titanium";
		else if (cmpIdentity.HasClass("QuartzDrill"))
			classe = "Quartz";

		let cmpRangeManager = Engine.QueryInterface(SYSTEM_ENTITY, IID_RangeManager);	
		let resources = cmpRangeManager.GetGaiaAndNonGaiaEntities();

		for (let i = 0; i < resources.length; ++i)
		{
			if (Engine.QueryInterface(resources[i], IID_Identity).HasClass(classe))
				Engine.QueryInterface(resources[i], IID_Visual).SelectAnimation(state, false, 1.0);
		}
	}
}
...
GuiInterface.prototype.SetBuildingPlacementPreview = function(player, cmd)
{
	...
	if (!this.placementEntity || this.placementEntity[0] != cmd.template)
	{
		...
		{
			this.DisplayArrow(this.placementEntity[1], "idle");
	...
	if (this.placementEntity)
	{
		...
		if (cmpVisual)
		{
			...
			if (!result.success)
			{
				this.DisplayArrow(ent, "click");
				cmpVisual.SetShadingColor(1.4, 0.4, 0.4, 1);
			}
			else
			{
				this.DisplayArrow(ent, "idle");
...
GuiInterface.prototype.IdleUnitFilter = function(unit, idleClasses, excludeUnits)
{
	...
	if (!cmpIdentity || cmpIdentity.HasClass("ResourceGatherer") != true)
		return { "idle": false };
...
let exposedFunctions = {
	...
	"SetEntitiesSelection" : 1,
	"SetTutorialIsLaunched" : 1,
	"GetTutorialIsLaunched" : 1
};
