...
Attack.prototype.PerformAttack = function(type, target)
{
	...
	if (cmpSound)
	{
		let soundName = cmpSound.GetSoundGroup("attack");
	...
	if (this.template[type].Projectile)
	{
		...
		data.position = new Vector3D(predictedPosition.x + offsetX, predictedHeight + 10, predictedPosition.z + offsetZ);
		...
		cmpTimer.SetTimeout(SYSTEM_ENTITY, IID_ProjectileManager, "RemoveProjectile", timeToTarget*rand*1000, data.projectileId);


		// Lancer un timer ou checker des collisions
		// Lorsque le timer est termin� ou que des collisions sont d�tect�es

		//cmpProjectileManager.RemoveProjectile(data.projectileId); // On supprime le projectile
