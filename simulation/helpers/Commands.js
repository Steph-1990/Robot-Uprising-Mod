...
var g_Commands = {
	...
	"drop-units": function(player, cmd, data)
	{
		GetFormationUnitAIs(data.entities, player, cmd, data.formation).forEach(cmpUnitAI =>
			cmpUnitAI.DropUnits(cmd.x, cmd.z, cmd.queued)
		);
	},
...
function notifyUnloadFailure(player)
{
	...
	cmpGUIInterface.PushNotification({
		"type": "text",
		"players": [player],
		"message": markForTranslation("Unable to unload unit(s). Land the spaceship before unloading."),
		"translateMessage": true
	});
}
